...
<html lang="fr">
	...
	<body>
		...
		<div id="game">
			...
			<span id="give-up" class="button" onclick="leave()">Revenir au menu</span>
		...
		<script>
			...
			var spectateMatchId = <?php
			    if (isset($_REQUEST['match']))
			        if ($_REQUEST['match']=='last') {
			            $result = sendRequest("SELECT id FROM MATCHES WHERE end=0 ORDER BY id DESC");
			            echo($result->num_rows==0 ? 'null' : $result->fetch_assoc()['id']);
			        } else echo $_REQUEST['match'];
			    else echo 'null';
			?>;
			window.addEventListener("load", function() {
				spectate(spectateMatchId);
				
				//setInterval(refreshTimer, 100);
				setInterval(function(){
					if (!match || match.ended || stop)
						return;
					getSpectateUpdate(spectateMatchId);
				}, 3000);
			});
				...
				if (response=="not") {
					...
					try{
						...
						if (!response.error) {
							...
							if (response.actions) {
								update(response.actions, response.from)/*.catch(function(e){
								    if(window.location.replace)
								        window.location.replace("spectate.php?match="+spectateMatchId+"&error="+encodeURIComponent(e.message)+"&file="+encodeURIComponent(e.fileName)+"&line="+encodeURIComponent(e.lineNumber));
				...
				while (historyIndex < match.history.length) {
					...
					switch (action.name) {
						...
						case "playplacecard":
							if (action.playerId == match.playerId) {
								if (typeof player.hand=='number') player.hand--;
								else player.hand.splice(action.index, 1)[0];
								player.mana -= action.card.cost;
						...
						case "draw":
							if (action.playerId == match.playerId) {
								...
								if (typeof match.opponents[action.playerId].hand == "number") match.opponents[action.playerId].hand++;
								else match.opponents[action.playerId].hand.push(action.card);
						...
						case "playeffectcard":
							if (action.playerId == match.playerId) {
								...
								player.mana -= action.card.cost;
								player.discard.push(action.card);
						...
						case "rescue":
							...
							card.hp = card.hpmax;
							if (typeof match.opponents[action.playerId].hand == "number") match.opponents[action.playerId].hand++;
							else match.opponents[action.playerId].hand.push(card);
			...
			function refresh() {
				...
				endTurnButton.innerText = "Tour de "+(match.names[match.playing]);
				//setTimeout(get, 2000);
				// nom des joueurs
				...
				// main du joueur
				let isPlayerHandVisible = typeof match.opponents[playerId].hand!="number";
				n = isPlayerHandVisible ? match.opponents[playerId].hand.length : match.opponents[playerId].hand;
				for (let i = 0; i<n || playerHandDivs[i]; i++) {
					let card = isPlayerHandVisible ? match.opponents[playerId].hand[i] : undefined;
					if (i>=n) {
						for (let ex of playerHandDivs.splice(i))
							ex.parentElement.removeChild(ex);
					} else if (!playerHandDivs[i]) {
						playerHandDivs.push(createGamePlacedCard(card?card.id:"back", {x:portrait?(i<5?i*20+50-20*Math.min(n-1,4)/2:(i-5)*20+100-20*((n-1)/2)):(i*10+50-10*((n-1)/2)), y:portrait?(i<5?95:105):100, edit:card, dataset:{zone:"my-hand"}}));
					} else {
						updateGamePlacedCard(playerHandDivs[i], {x:portrait?(i<5?i*20+50-20*Math.min(n-1,4)/2:(i-5)*20+100-20*((n-1)/2)):(i*10+50-10*((n-1)/2)), y:portrait?(i<5?95:105):100, id:card?card.id:undefined, edit:card, dataset:{zone:"my-hand"}});
					}
				}
